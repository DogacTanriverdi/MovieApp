...
class SeeAllWatchListMoviesAdapter(
    private val onClick: (WatchListEntity) -> Unit
) : RecyclerView.Adapter<SeeAllWatchListMoviesAdapter.SeeAllWatchListMoviesViewHolder>() {

    inner class SeeAllWatchListMoviesViewHolder(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SeeAllWatchListMoviesViewHolder {
        val binding = SeeAllItemBinding.inflate(
            LayoutInflater.from(parent.context), parent, false
        )
        return SeeAllWatchListMoviesViewHolder(binding)
    }

    override fun getItemCount(): Int = movies.size

    override fun onBindViewHolder(holder: SeeAllWatchListMoviesViewHolder, position: Int) {
