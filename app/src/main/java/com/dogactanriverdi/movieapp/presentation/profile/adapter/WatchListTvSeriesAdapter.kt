...
class WatchListTvSeriesAdapter(
    private val onClick: (WatchListEntity) -> Unit
) : RecyclerView.Adapter<WatchListTvSeriesAdapter.WatchListTvSeriesViewHolder>() {

    inner class WatchListTvSeriesViewHolder(
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): WatchListTvSeriesViewHolder {
        val binding = MovieItemBinding.inflate(
            LayoutInflater.from(parent.context), parent, false
        )
        return WatchListTvSeriesViewHolder(binding)
    }

    override fun getItemCount(): Int = movies.size

    override fun onBindViewHolder(holder: WatchListTvSeriesViewHolder, position: Int) {
